...
@implementation XDABContactEntity
...
{
    return [[XDABContactEntity alloc] initWithRecord:person];
}
...
+ (id) contactWithRecordID: (ABRecordID) recordID
{
    ...
    XDABContactEntity *contact = [self contactWithRecord:contactrec];
    return contact;
}
...
+ (id) contact
{
    ...
    id contact = [XDABContactEntity contactWithRecord:person];
    CFRelease(person);
    return contact;
}
...
- (BOOL) addAddress: (NSDictionary *) dictionary
{
    ...
    if (![XDABContactEntity isMultivalueDictionary:dictionary]) return NO;
...
{
    if (!dictionary) return NO;
    if ([XDABContactEntity isMultivalueDictionary:dictionary])
        return [self addAddress:dictionary];
    
    NSDictionary *multi = [XDABContactEntity dictionaryWithValue:dictionary andLabel:label];
...
{
    if (!value) return NO;
    NSDictionary *multi = [XDABContactEntity dictionaryWithValue:value andLabel:label];
...
- (ABMutableMultiValueRef) copyMultiValueFromArray: (NSArray *) anArray withType: (ABPropertyType) aType
{
    ...
    {
        if (![XDABContactEntity isMultivalueDictionary:dict])
            continue;
...
{
    XDABContactEntity *contact = [XDABContactEntity contact];
