...
{
    return [self fetchObjectsForEntity:entity
                             predicate:nil
                       sortDescriptors:nil];
}
...
{
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntity:[NSEntityDescription entityForName:entity inManagedObjectContext:self]
                                                           predicate:predicate
                                                     sortDescriptors:sortDescriptors];
    if (limit > 0)
...
{
    return [self fetchObjectForEntity:entity
                            predicate:nil
                      sortDescriptors:nil];
}
...
- (id)fetchObjectForEntity:(NSString *)entity
           ...
           sortDescriptors:(NSArray *)sortDescriptors
{
    NSArray *results = [self fetchObjectsForEntity:entity
                                         predicate:predicate
                                   sortDescriptors:sortDescriptors];
    if (results.count < 1)
        return nil;
...
- (void)fetchObjectsForEntity:(NSString *)entity
                     callback:(FetchObjectsCallback)callback
{
    [self fetchObjectsForEntity:entity
                      predicate:nil
                sortDescriptors:nil
                       callback:callback];
}
                     ...
                     callback:(FetchObjectsCallback)callback
{
    ...
    NSFetchRequest *request = [[NSFetchRequest alloc] initWithEntity:entityDescription
                                                           predicate:predicate
                                                     sortDescriptors:sortDescriptors];
...
- (void)fetchObjectForEntity:(NSString *)entity
                    callback:(FetchObjectCallback)callback
{
...
- (void)fetchRequest:(NSFetchRequest *)fetchRequest
        withCallback:(FetchObjectsCallback)callback
{
       ...
       withPredicate:(NSPredicate *)predicate
{
    ...
    NSFetchRequest *fetchRequest = [NSFetchRequest fetchRequestWithEntity:[NSEntityDescription entityForName:entity inManagedObjectContext:self]
                                                                predicate:predicate];
