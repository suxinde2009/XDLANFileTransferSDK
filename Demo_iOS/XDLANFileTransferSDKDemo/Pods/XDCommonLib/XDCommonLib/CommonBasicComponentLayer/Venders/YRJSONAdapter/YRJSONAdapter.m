...
+ (void)swizzleInstanceMethodForClass:(Class)class
                               origin:(SEL)origin
                              replace:(SEL)replace
{
...
+ (void)load {
    [self swizzleInstanceMethodForClass:[NSString class]
                                 origin:@selector(objectFromJSONString)
                                replace:@selector(yr_objectFromJSONString)];
    
    [self swizzleInstanceMethodForClass:[NSString class]
                                 origin:@selector(mutableObjectFromJSONString)
                                replace:@selector(yr_mutableObjectFromJSONString)];
    
    [self swizzleInstanceMethodForClass:[NSData class]
                                 origin:@selector(objectFromJSONData)
                                replace:@selector(yr_objectFromJSONData)];
    
    [self swizzleInstanceMethodForClass:[NSData class]
                                 origin:@selector(mutableObjectFromJSONData)
                                replace:@selector(yr_mutableObjectFromJSONData)];
    
    [self swizzleInstanceMethodForClass:[NSString class]
                                 origin:@selector(JSONData)
                                replace:@selector(yr_JSONData)];
    
    [self swizzleInstanceMethodForClass:[NSString class]
                                 origin:@selector(JSONString)
                                replace:@selector(yr_JSONString)];
    
    [self swizzleInstanceMethodForClass:[NSArray class]
                                 origin:@selector(JSONData)
                                replace:@selector(yr_JSONData)];
    ...
    [self swizzleInstanceMethodForClass:[NSDictionary class]
                                 origin:@selector(JSONData)
                                replace:@selector(yr_JSONData)];
